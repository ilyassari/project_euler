...
def largest_product(grid, length):
    """ This function only works for length = 4. This should be improved """
    l_prod = 0
    for row in grid:
    ...
    for i in range(20):
        ...
        for row in range(i+1):
            for col in range(i+1):
                if row + col == i:
                    grid_NW[i].append(grid[row][col])
    for i in range(20, 39):
        grid_NW.append(list())
        for row in range((i - 19), 20):
            for col in range(19, (i-20), -1):
                if row + col == i:
                    grid_NW[i].append(grid[row][col])
    for nw in grid_NW:
        if len(nw) >= length:
            for i in range(len(nw)-length + 1):
                prod = nw[i] * nw[i + 1] * nw[i + 2] * nw[i + 3]
                if prod > l_prod:
                    l_prod = prod
    grid_SW = list()
    for i in range(20):
        grid_SW.append(list())
        for row in range(19, (19-i-1), -1):
            for col in range(i, -1, -1):
                if row - col == (19 - i):
                    grid_SW[i].append(grid[row][col])
    for i in range(20, 39):
        grid_SW.append(list())
        for row in range((38-i), -1, -1):
            for col in range(19, (i - 20), -1):
                if row-col == (19 - i):
                    grid_SW[i].append(grid[row][col])
    for sw in grid_SW:
        if len(sw) >= length:
            for i in range(len(sw)-length+1):
                prod = sw[i] * sw[i + 1] * sw[i + 2] * sw[i + 3]
                if prod > l_prod:
                    l_prod = prod
    return l_prod


print(largest_product(grid, 4))  #70600674
